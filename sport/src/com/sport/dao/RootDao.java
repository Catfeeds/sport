...
public class RootDao {
	...
	public <T> int find(String queryString, List<T> members, int pageNumber,
			int pageSize, String groupByColumn, String orderByColumn,
			boolean isAsc) {
		try{
		// 分组
		...
		if (orderByColumn == null || orderByColumn.trim().equals(""))
			...
			queryString += " order by e." + orderByColumn
					+ (isAsc ? " asc" : " desc");
		// 获取总的记录数
		int count = hibernateTemplate.find(queryString).size();
		

		List<T> copyMembers = (List<T>) (hibernateTemplate.getSessionFactory()
	...
	//修改字符串类型的字段
		public boolean alterColString(String entityName,int id,String colName, String colValue) {
			...
			Query query=hibernateTemplate.getSessionFactory().
					getCurrentSession().createQuery(sql).setString(0, colValue);		
			if(query.executeUpdate()>0)
				return true;
			return false;
		}
		...
		public boolean alterDate(String entityName,int id,String colName, Date date) {
			...
			String sql="update "+entityName+" e set e."+colName+"=? where e.id = "+id;
			Query query=hibernateTemplate.getSessionFactory().
					getCurrentSession().createQuery(sql).setDate(0, date);		
			if(query.executeUpdate()>0)
				return true;
			return false;
		}
