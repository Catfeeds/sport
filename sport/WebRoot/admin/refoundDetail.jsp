...
<html>
	...
	<body>
		<div class="finance-mainbody">
			...
			<p>订单状态：
				<s:if test="refound.order.orderStatus==8">
					正在申请退款
				</s:if>
			...
			<div class="finance-settlement-div">
				<span>联系电话：&emsp;&emsp;</span>
				<s:property value="refound.phone" /> 
			</div>
			<div class="finance-settlement-div">
				<span>姓&emsp;&emsp;名：&emsp;&emsp;</span><s:property value="refound.realName"/> 
			</div>
			<div class="finance-settlement-div">
				<span>支付宝帐号：&emsp;</span><s:property value="refound.zifubaoAcount"/> 
			</div>
			<div class="finance-settlement-div">
				<span>微信支付帐号：&emsp;</span><s:property value="refound.weixinAcount"/> 
			</div>
			<div class="finance-settlement-div">
				<table>
					<tr><td><span style="text-align: center;top: 0;">退款原因:&nbsp;&nbsp;&nbsp;&nbsp;&emsp;</span></td>
			...
			<s:if test="!refound.hasRefound">
			<div class="finance-settlement-button">
				<a id="ensureBtn" refoundId="<s:property value='refound.id'/>" href="javascript:void(0);" >确认已退款</a>
			</div>
			</s:if>
		...
		<script type="text/javascript">
			$(function(){
				$("#ensureBtn").click(function(){
					...
					$.post("refoundandensureRefounded", {"refound.id":refoundId},
					   function(data){
					   	 var arr=new Array();
							...
							if(arr[0]){//true时表示操作成功，更改界面元素状态								
								window.document.location="admin/refoundandrefoundList";
							}else{
								alert(arr[1]);
							}	
					 },'json');
