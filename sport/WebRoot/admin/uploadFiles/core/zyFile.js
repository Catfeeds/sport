...
var ZYFILE = {
		...
		url : "imageanduploadImages",  					  // 上传action路径
		...
		funUploadFiles : function(){
			...
			$.each(this.uploadFile, function(k, v){
				//alert('aaa');
				self.funUploadFile(k+1,v);
			});
		},
		// 上传单个个文件
		funUploadFile : function(picIndex,file){
			var self = this;  // 在each中this指向没个v  所以先将this保留
			
			var formdata = new FormData();
			formdata.append(self.imgName, file);	
			formdata.append("type", self.imgType);
			formdata.append("productId",self.productId);//产品对应的id
			//图片的索引，用于给图片重新命名
			formdata.append("picNumber", picIndex);
		    ...
		    xhr.addEventListener("load", function(e){
		    	...
		    	// 回调到外部
		    	self.onSuccess(file, xhr.responseText,self.imgType);
		    	if(self.uploadFile.length==0){
		    		// 回调全部完成方法
		    		self.onComplete(file, xhr.responseText,self.imgType);
		    	}
		    }, false);  
		    // 错误
