...
public class AddressService extends RootService {
	...
	public void add(Address paddr) throws RootException, Exception {
		if (paddr == null
				|| (paddr.getAddressName() == null || (paddr.getAddressName()
						.trim().equals(""))))
	...
	public void update(Address paddr) throws RootException, Exception {
		...
		try {
			...
			oldpaddr.setAddressName(paddr.getAddressName())
					.setIntroduction(paddr.getIntroduction());
			...
			if (!(addressDao.load(oldpaddr.getParentAddress()).getId() == (oldAddressId))) {
				...
				Address newPPaddr = addressDao.load(oldpaddr.getParentAddress()
						.getId());
				if (oldPPaddr.getChildrenAddress() == null
						|| oldPPaddr.getChildrenAddress().isEmpty())
	...
	// 查询某根地区下的所有子地区集
	public List<Address> findChildAddress(Address addr) throws RootException,
			Exception {
	...
	public List<Address> findChildAddress(int id) throws RootException,
			Exception {
		if (id == -1)
			return addressDao.loadRootAddress();
	...
	public int findByAddress(List<Address> addrs, Address addr, int pageNumber,
			int pageSize) {
