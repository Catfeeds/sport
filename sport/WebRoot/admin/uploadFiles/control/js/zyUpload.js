	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "imageanduploadImages",  	// 上传文件的路径
					multiple         : true,  						// 是否可以多个文件上传
					dragDrop         : true,  						// 是否可以拖动上传文件
					del              : true,  						// 是否可以删除文件
					finishDel        : false,  						// 是否在上传文件完成后删除预览
					imgType			 :"midpic",						//图片类型
					imgName			 :"img",							//上传后接收该文件的File名
					productId		 :"0",							//0代表是新添加的产品
					/* 提供给外部的接口方法 */
			...
			this.createHtml = function(){
				...
				var imgType="";
				para.imgType? imgType=para.imgType : imgType="midimg";
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input id="fileType" type="hidden" name="type" value="'+imgType+'">';
	            	html += '						<input id="productId" type="hidden" name="type" value="'+para.productId+'">';
	            	html += '						<input id="fileImage" type="file" size="30" name="'+para.imgName+'" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					imgType :$("#fileType").attr("value"),			  //图片类型
					imgName:$("#fileImage").attr("name"),			  //文件对应接收的文件对象名 
					productId:$("#productId").attr("value"),//接收产品ID，0代表新产品，否则为已有产品
					...
					onSuccess:function(file, response,type) {
						//alert("parent");
						para.onSuccess(file,response,type);
					...
					onFailure:para.onFailure/* function(file) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#uploadInf").append("<p>文件" + file.name + "上传失败！</p>");	
						//$("#uploadImage_" + file.index).css("opacity", 0.2);
					}*/,
					onComplete: function(file, response,type) {
						//alert("parent");
						para.onComplete(file,response,type);
						/*$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
