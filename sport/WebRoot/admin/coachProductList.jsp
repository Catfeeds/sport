 ...
 // 通过原生select控件创建自定义下拉框
   ...
   var ddl_picture2=createDropList("#typeParent");
	...
	ddl_picture2.change(function(){	
		...
		if(typeId!=-1)
			document.location="coachProductandcoachProductList?product.type.id="+typeId+"&page.pageNumber=1";	
	});
...
function operateNoReturn(operate,id){
	cancelFlag=false;
	if(operate=="coachProductanddeleteCoachProduct")
	{
		var txt=  "确定要删除该产品？";
		var option = {
			title: "提示：",
...
function deleteBatch(){
		...
		var option = {
			...
			btn: parseInt("0011",2),
			onOk: function(){	//回调函数			
					$.post("coachProductanddeleteSelectedProducts",
					 	{"ids":ids},
						function(data){
							var arr=new Array();
							$.each(data, function(i, item) {
		...
		<div class="title">选择查看某分类下的产品:</div>
			...
			<div class="item" >
				<a  class="button blue middle" id="getAllProductsBtn" 
					href="coachProductandcoachProductList?page.pageNumber=1"
					style="margin-top:5px;margin-left:20px;" >&nbsp;所有产品&nbsp;&nbsp;</a>
			</div>
			
			<div class="item" >
				<a  class="button blue middle" id="getAllProductsBtn" 
					href="coachProductandcoachProductList?page.pageNumber=1&product.hasTop=true"
					style="margin-top:5px;margin-left:20px;" >已置顶产品</a>
			</div>
			<div class="item" >
				<a  class="button blue middle" id="getAllProductsBtn" 
					href="coachProductandcoachProductList?page.pageNumber=1&product.hasBegin=true"
				...
				<div id="productItem<s:property value='id'/>" class="item">
					<div class="productInfoWrap">
						<div class="productImg">
							<a href="coachProductandcoachProductDetail?product.id=<s:property value='id'/>" >
						...
						<div class="productBaseInfoWrap">
							...
							<div class="price">
								普通价：<span class="moneyStyle"><s:property value="normalPrice"/>&yen;</span>&nbsp;(每小时)
								促销价：<span class="moneyStyle"><s:property value="bargainPrice"/>&yen;</span>&nbsp;(每小时)
							</div>
							
							<div class="stock">
								学员标准人数：<span class="contentStyle"><s:property value="employNumber"/>人 &nbsp;</span>
							</div>
							<div class="stock">
					...
					<div class="productOperate">
						...
						<span id="getQrcodeBtn">
							...
							<a href="javascript:operateNoReturn3('coachProductandtopProduct',<s:property value='id'/>,
								'product.hasTop'
							...
							<a href="javascript:operateNoReturn3('coachProductanddisableProduct',<s:property value='id'/>,
							'product.hasBegin'
							...
							<a href="javascript:operateNoReturn('coachProductanddeleteCoachProduct',<s:property value='id'/>);" 
	...
	<div class="pageProducts">
		<div class="pagination">
			...
			<ul>
				...
				<s:else>
					<li><a
						href="coachProductandcoachProductList?type.id=<s:property value='type.id'/>&page.pageNumber=1">首页</a>

					</li>
					<li><a
						href="coachProductandcoachProductList?type.id=<s:property value='type.id'/>&page.pageNumber=<s:property value='page.pageNumber-1'/>">上一页</a>
					</li>
				</s:else>
				<!-- 如果总页码小于7页，就显示全部页码 -->
				<s:if test="page.totalPageNumber<=7">
					<s:iterator begin="1" var="i" end="page.totalPageNumber">
						...
						<s:else>
							<li><a
								href="coachProductandcoachProductList?type.id=<s:property value='type.id'/>&page.pageNumber=<s:property/>">
									<s:property /> </a>
							</li>
						</s:else>
					</s:iterator>
				...
				<s:else>
					<li class="nextpage"><a
						href="coachProductandcoachProductList?type.id=<s:property value='type.id'/>&page.pageNumber=<s:property value='page.pageNumber+1'/>">下一页</a>
					</li>
					<li class="nextpage"><a
						href="coachProductandcoachProductList?type.id=<s:property value='type.id'/>&page.pageNumber=<s:property value='page.totalPageNumber'/>">尾页</a>
					</li>
				</s:else>
			</ul>
			</s:if>
