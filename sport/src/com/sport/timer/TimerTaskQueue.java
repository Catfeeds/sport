...
public class TimerTaskQueue {
    ...
    public final static long CHECK_NOTPAY_ORDER = 1000 * 60 * 20;// 每20分钟检查是否有超过20分钟未支付的订单,如果有，就释放其预定的场地等信息
    ...
    public TimerTaskQueue() {
        ...
        unablePreOrderTask = new TimerTask() {
            @Override
            public void run() {
                System.out.println("调用将当前时间之前的预订信息置为不可用的方法，此时的时间是：" + new Date());
    ...
    public void updateRecordUpdateTime() {
        ...
        try {
            ...
            if (update != null) {
                update.setDate(new Date()).setUpdateNumber(update.getUpdateNumber() + 1);
    ...
    public void runTimeTask() {
        ...
        date.set(date.get(Calendar.YEAR), date.get(Calendar.MONTH), date.get(Calendar.DAY_OF_MONTH), HOUR, MINUTE, SECEND);
        ...
        if (hostConfigService.isTimerFlag()) {
            // 每个30分钟检查一次订单支付超时
            timer.schedule(checkOrderTimeOutTask, Calendar.getInstance().getTime(), CHECK_NOTPAY_ORDER);// 立即执行

            Calendar date1 = Calendar.getInstance();
            date1.set(date1.get(Calendar.YEAR), date1.get(Calendar.MONTH), date1.get(Calendar.DAY_OF_MONTH),
                date1.get(Calendar.HOUR_OF_DAY), 0, 0);
            // 每小时重置一次时间已经过去的预定信息
            timer.schedule(unablePreOrderTask, date1.getTime(), UPDATE_PREORDER_SCHEDULE);// 立即执行
        }
    }
    ...
    @Resource
    public void setUpdateProgressService(UpdateProgressService updateProgressService) {
